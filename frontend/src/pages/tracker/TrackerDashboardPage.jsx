...
export default function TrackerDashboardPage() {
  ...
  const d = data || {};
  // Backend returns flat fields, not a stats sub-object
  ...
  return (
    <div style={{ padding: "24px 32px", maxWidth: 1200 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
        ...
        />
        <StatCard value={pendingDecisions.length} label="Pending Decisions" accent={theme.accent.purple} />
      </div>
      ...
      <div style={{ ...cardStyle, marginBottom: 24 }}>
        ...
        {deadlines.length === 0 ? (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {["Matter", "Type", "Deadline", "Priority"].map((h) => (
                  <th key={h} style={{
            ...
            <tbody>
              {deadlines.map((dl, i) => {
                const deadline = dl.external_deadline || dl.decision_deadline || dl.work_deadline;
                const dlType = dl.external_deadline ? "External" : dl.decision_deadline ? "Decision" : "Work";
                return (
                  <tr key={dl.id || i} style={{ cursor: "pointer" }}
                    onClick={() => dl.id && navigate(`/matters/${dl.id}`)}
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: theme.text.faint, marginTop: 2 }}>{m.owner_name || "\u2014"}</div>
                  </div>
          ...
          {recentUpdates.length === 0 ? (
            ...
            recentUpdates.map((u, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 11, color: theme.text.faint, marginTop: 3 }}>{formatDate(u.created_at)}</div>
              </div>
            ))
          )}
        </div>
