...
export default function ItemDetailPage() {
  ...
  const [linkedProjectLoading, setLinkedProjectLoading] = useState(true);
  ...
  // Check for linked work project
  useEffect(() => {
    if (!id) return;
    setLinkedProjectLoading(true);
    getProjectByPipelineItem(id)
  ...
  const handleCreateProject = async () => {
    ...
    try {
      const result = await createProjectFromItem(id, createProjectForm);
      toast.success("Work project created");
      setLinkedProject(result);
      setShowCreateProject(false);
      setCreateProjectForm({ priority_label: "medium", apply_template: true });
    } catch (e) {
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          {linkedProject && (
            <button onClick={() => navigate(`/work/${linkedProject.id}`)} style={actionBtn("#059669")}>View Project →</button>
          )}
      ...
      <div style={{ background: theme.bg.card, borderRadius: 10, border: `1px solid ${theme.border.default}`, padding: "16px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        ...
        {linkedProject && (
          <>
            <Divider />
            <div style={{ cursor: "pointer" }} onClick={() => navigate(`/work/${linkedProject.id}`)}>
              <div style={{ fontSize: 10, color: theme.text.faint, marginBottom: 2 }}>Work Project</div>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#a78bfa" }}>{linkedProject.title}</div>
            </div>
          </>
        )}
      </div>
      ...
      {/* Create Work Project */}
      <Modal isOpen={showCreateProject} onClose={() => setShowCreateProject(false)} title="Create Work Project" width={440}>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <p style={{ color: theme.text.muted, fontSize: 13, lineHeight: 1.5, margin: 0 }}>
            Create a work management project linked to <strong style={{ color: theme.text.secondary }}>{item.title}</strong>.
            The project will be pre-populated with this item's title, description, and lead attorney.
          </p>
          <div>
            <label style={labelStyle}>Priority</label>
            <select style={inputStyle} value={createProjectForm.priority_label}
              onChange={e => setCreateProjectForm(f => ({ ...f, priority_label: e.target.value }))}>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: theme.text.muted, cursor: "pointer" }}>
            <input type="checkbox" checked={createProjectForm.apply_template}
              onChange={e => setCreateProjectForm(f => ({ ...f, apply_template: e.target.checked }))} />
            Apply rulemaking template
          </label>
          <button onClick={handleCreateProject} disabled={creatingProject}
            style={{ ...actionBtn("#7c3aed"), width: "100%", opacity: creatingProject ? 0.5 : 1 }}>
