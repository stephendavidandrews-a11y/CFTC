...
import { getWorkDashboard } from "../api/work";
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
        ...
        <div
          style={{ background: theme.bg.card, borderRadius: 10, border: `1px solid ${theme.border.default}`, padding: 20, cursor: "pointer" }}
          onClick={() => navigate("/work")}
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <h3 style={{ fontSize: 13, fontWeight: 700, color: theme.text.primary, margin: 0 }}>Work Management</h3>
            <span style={{ fontSize: 11, color: theme.accent.blueLight, fontWeight: 600 }}>View all &rarr;</span>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ background: theme.bg.cardHover, borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ fontSize: 22, fontWeight: 700, color: theme.text.primary }}>{totalActiveProjects}</div>
              <div style={{ fontSize: 10, color: theme.text.faint }}>Active Projects</div>
            </div>
            <div style={{ background: theme.bg.cardHover, borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ fontSize: 22, fontWeight: 700, color: theme.accent.blue }}>{workInProgress}</div>
              <div style={{ fontSize: 10, color: theme.text.faint }}>In Progress</div>
            </div>
            <div style={{ background: theme.bg.cardHover, borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ fontSize: 22, fontWeight: 700, color: wd.blocked_items > 0 ? theme.accent.red : theme.text.primary }}>{wd.blocked_items}</div>
              <div style={{ fontSize: 10, color: theme.text.faint }}>Blocked</div>
            </div>
            <div style={{ background: theme.bg.cardHover, borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ fontSize: 22, fontWeight: 700, color: theme.accent.green }}>{workCompleted}</div>
              <div style={{ fontSize: 10, color: theme.text.faint }}>Completed</div>
            </div>
          </div>
          {(tasksTodo > 0 || tasksOverdue > 0) && (
            <div style={{ display: "flex", gap: 12, marginTop: 12, paddingTop: 12, borderTop: `1px solid ${theme.border.subtle}` }}>
              <span style={{ fontSize: 11, color: theme.text.dim }}>{tasksTodo} task{tasksTodo !== 1 ? "s" : ""} pending</span>
              {tasksOverdue > 0 && (
                <span style={{ fontSize: 11, color: theme.accent.red, fontWeight: 600 }}>{tasksOverdue} overdue</span>
              )}
              {wd.overdue_items > 0 && (
                <span style={{ fontSize: 11, color: theme.accent.red, fontWeight: 600 }}>{wd.overdue_items} work item{wd.overdue_items !== 1 ? "s" : ""} overdue</span>
              )}
            </div>
          )}
        </div>
