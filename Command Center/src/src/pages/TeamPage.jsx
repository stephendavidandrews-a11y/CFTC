...
import { listTeam, getTeamDashboard, getMemberWorkload, createTeamMember, updateTeamMember } from "../api/pipeline";
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 11, fontWeight: 700, color: theme.text.faint, textTransform: "uppercase", marginBottom: 10 }}>
                          Active Items ({workload.active_items?.length || 0})
                        </div>
                        {(workload.active_items || []).length === 0 ? (
                          <div style={{ fontSize: 12, color: theme.text.dim, fontStyle: "italic" }}>No active items</div>
                        ) : (
                          (workload.active_items || []).map((item) => (
                            <div
                              key={item.id}
                              onClick={() => {
                      ...
                      <div>
                        ...
                        {(workload.overdue_deadlines || []).length > 0 && (
                          <div style={{ marginBottom: 8 }}>
                            <div style={{ fontSize: 10, fontWeight: 600, color: theme.accent.red, marginBottom: 4 }}>OVERDUE</div>
                            {workload.overdue_deadlines.map((dl, i) => (
                              <div key={i} style={{
                                padding: "6px 10px", marginBottom: 3, borderRadius: 4,
                                background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)",
                                fontSize: 11, color: theme.accent.redLight,
                              }}>
                        ...
                        {(workload.upcoming_deadlines || []).length === 0 && (workload.overdue_deadlines || []).length === 0 ? (
                        ...
                        <div style={{ marginTop: 16 }}>
                          ...
                          {(() => {
                            ...
                            const active = workload.active_items?.length || 0;
                            const pct = Math.min((active / max) * 100, 100);
                            const color = pct >= 90 ? theme.accent.red : pct >= 70 ? theme.accent.yellow : theme.accent.green;
                            return (
                              <div>
