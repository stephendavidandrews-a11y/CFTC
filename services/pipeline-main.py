...
def verify_auth(credentials: HTTPBasicCredentials = Depends(security)):
    ...
    correct_user = secrets.compare_digest(
        credentials.username.encode(), settings.PIPELINE_USER.encode()
    )
    correct_pass = secrets.compare_digest(
        credentials.password.encode(), settings.PIPELINE_PASS.encode()
    )
...
def _mark_run(conn, task_key: str, run_status: str = "ok"):
    ...
    conn.execute(
        ...
        (task_key, now, run_status, now, run_status),
    )
    conn.commit()
...
# ── Pipeline routes (all auth-protected) ──────────────────────────────────

PIPELINE_PREFIX = "/pipeline"
app.include_router(pipeline_items_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_deadlines_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_team_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_documents_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_dashboard_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_stakeholders_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_integrations_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_contacts_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_interagency_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_ai_router, prefix=PIPELINE_PREFIX, dependencies=_auth)
app.include_router(pipeline_loper_router, prefix=PIPELINE_PREFIX, dependencies=_auth)


# ── Work Management routes (all auth-protected) ──────────────────────────

WORK_PREFIX = "/pipeline/work"
app.include_router(work_projects_router, prefix=WORK_PREFIX, dependencies=_auth)
app.include_router(work_items_router, prefix=WORK_PREFIX, dependencies=_auth)
app.include_router(work_tasks_router, prefix=WORK_PREFIX, dependencies=_auth)
app.include_router(work_notes_router, prefix=WORK_PREFIX, dependencies=_auth)
app.include_router(work_dashboard_router, prefix=WORK_PREFIX, dependencies=_auth)
app.include_router(work_templates_router, prefix=WORK_PREFIX, dependencies=_auth)


# ── Health check (intentionally unauthenticated for monitoring) ───────────

@app.get("/health")
async def health_check():
    return {"status": "ok", "version": "0.3.0", "system": "CFTC Regulatory Pipeline Manager"}
