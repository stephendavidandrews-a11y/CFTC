...
def _resolve_pipeline_info(conn: sqlite3.Connection, pipeline_id: int | None) -> dict:
    """Look up pipeline item info (title, rin, stage) from attached pipeline DB."""
    result = {
        "linked_pipeline_title": None,
        "linked_pipeline_rin": None,
        "linked_pipeline_stage": None,
    }
    if not pipeline_id:
        return result
    try:
        row = conn.execute(
            """SELECT pi.title, pi.rin, pi.current_stage,
                      st.stage_label
               FROM pipeline.pipeline_items pi
               LEFT JOIN pipeline.stage_templates st
                 ON st.module = pi.module
                 AND st.item_type = pi.item_type
                 AND st.stage_key = pi.current_stage
               WHERE pi.id = ?""",
            (pipeline_id,),
        ).fetchone()
        if row:
            result["linked_pipeline_title"] = row["title"]
            result["linked_pipeline_rin"] = row["rin"]
            result["linked_pipeline_stage"] = row["stage_label"] or row["current_stage"]
    except Exception:
        pass
    return result
...
def _enrich_project(row: sqlite3.Row, conn: sqlite3.Connection) -> dict:
    ...
    # Enrich with pipeline item info if linked
    pipeline_info = _resolve_pipeline_info(conn, d.get("linked_pipeline_id"))
    d.update(pipeline_info)

    # Compute delegation percentage
    d["delegation_pct"] = _compute_delegation_pct(conn, d["id"])

    # Ensure new fields have defaults
    d.setdefault("source", None)
    d.setdefault("why_me", None)
    d.setdefault("completion_notes", None)
    d.setdefault("due_date", None)

    return d


def _compute_delegation_pct(conn: sqlite3.Connection, project_id: int) -> int:
    """Compute percentage of work items that are assigned to team members."""
    total = conn.execute(
    ...
    assigned = conn.execute(
        """SELECT COUNT(DISTINCT wi.id) FROM work_items wi
           ...
           WHERE wi.project_id = ? AND wi.status != 'completed'""",
        (project_id,)
    ).fetchone()[0]
...
def get_project_by_pipeline_item(conn: sqlite3.Connection, pipeline_item_id: int) -> dict | None:
    """Get the project linked to a given pipeline item."""
    row = conn.execute(
        "SELECT * FROM projects WHERE linked_pipeline_id = ?", (pipeline_item_id,)
    ).fetchone()
    if not row:
        return None
    return _enrich_project(row, conn)
