...
def seed_all(conn: sqlite3.Connection) -> None:
    ...
    if row[0] > 0:
        logger.info("Seed skipped — organizations table already has data (%d rows).", row[0])
        return
    ...
    orgs = {
        ...
        "cftc":      (cftc_id, "Commodity Futures Trading Commission", "CFTC", "CFTC office", None),
        # Chairman & Commissioners — child of CFTC
        "chairman":  (_uid(), "Office of Chairman Selig", None, "Commissioner office", cftc_id),
        # Divisions — all children of CFTC
        "ogc":       (_uid(), "Office of the General Counsel", "OGC", "CFTC office", cftc_id),
        "dcr":       (_uid(), "Division of Clearing and Risk", "DCR", "CFTC division", cftc_id),
        "doe":       (_uid(), "Division of Enforcement", "DOE", "CFTC division", cftc_id),
        "dmo":       (_uid(), "Division of Market Oversight", "DMO", "CFTC division", cftc_id),
        "mpd":       (_uid(), "Market Participants Division", "MPD", "CFTC division", cftc_id),
        ...
        "oia":       (_uid(), "Office of International Affairs", "OIA", "CFTC office", cftc_id),
        "opa":       (_uid(), "Office of Public Affairs", "OPA", "CFTC office", cftc_id),
        "olia":      (_uid(), "Office of Legislative and Intergovernmental Affairs", "OLIA", "CFTC office", cftc_id),
        "oceo":      (_uid(), "Office of Customer Education and Outreach", "OCEO", "CFTC office", cftc_id),
        "oig":       (_uid(), "Office of the Inspector General", "OIG", "CFTC office", cftc_id),
        # External agencies (no parent)
        "sec":       (_uid(), "Securities and Exchange Commission", "SEC", "Federal agency", None),
        "treasury":  (_uid(), "Department of the Treasury", "Treasury", "Federal agency", None),
        "omb":       (_uid(), "Office of Management and Budget", "OMB", "White House / OMB", None),
        # Exchanges / industry (no parent)
        "cme":       (_uid(), "CME Group", "CME", "Exchange", None),
        "ice":       (_uid(), "Intercontinental Exchange", "ICE", "Exchange", None),
        "fia":       (_uid(), "Futures Industry Association", "FIA", "Trade association", None),
    }
    ...
    conn.commit()
    logger.info(
        "Seed complete — %d orgs, %d people. "
        "Matters will be created by pipeline sync.",
        len(orgs), len(people),
    )
