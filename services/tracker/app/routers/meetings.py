...
async def list_meetings(
    ...
    if matter_id:
        conditions.append(
            "m.id IN (SELECT mm.meeting_id FROM meeting_matters mm WHERE mm.matter_id = ?)"
        )
        params.append(matter_id)
    where = "WHERE " + " AND ".join(conditions) if conditions else ""

    total = db.execute(
        f"SELECT COUNT(*) as c FROM meetings m {where}", params
    ).fetchone()["c"]
    rows = db.execute(
        f"""
...
async def create_meeting(
    ...
    """,
        (
            ...
            body.notes,
            body.decisions_made,
            body.readout_summary,
            body.created_followups,
            body.boss_attends,
            body.external_parties_attend,
    ...
    new_data.update(
        {"id": mid, "source": source_val, "created_at": now, "updated_at": now}
    )
    log_event(
        db,
        table_name="meetings",
...
async def update_participant(
    meeting_id: str, participant_id: str, body: dict, db=Depends(get_db)
):
    ...
    db.execute(
        f"UPDATE meeting_participants SET {set_clause} WHERE id = ? AND meeting_id = ?",
        params,
    )
    db.commit()
