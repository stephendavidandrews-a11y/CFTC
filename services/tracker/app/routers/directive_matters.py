...
async def create_directive_matter(
    ...
    if not directive:
        raise HTTPException(status_code=404, detail="Policy directive not found")
    matter = db.execute(
        "SELECT id FROM matters WHERE id = ?", (body.matter_id,)
    ).fetchone()
    if not matter:
        raise HTTPException(status_code=404, detail="Matter not found")
    ...
    if existing:
        raise HTTPException(
            status_code=409, detail="This directive is already linked to this matter"
        )
...
async def delete_directive_matter(
    ...
    old = db.execute(
        "SELECT * FROM directive_matters WHERE id = ?", (link_id,)
    ).fetchone()
    if not old:
