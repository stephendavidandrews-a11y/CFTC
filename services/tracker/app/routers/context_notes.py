...
def _validate_enums(data: dict, is_create: bool = True):
    ...
    if "category" in data and data["category"] not in VALID_CATEGORIES:
        raise HTTPException(
            400,
            detail=f"Invalid category '{data['category']}'. Must be one of: {sorted(VALID_CATEGORIES)}",
        )
    if "posture" in data and data["posture"] not in VALID_POSTURES:
        raise HTTPException(
            400,
            detail=f"Invalid posture '{data['posture']}'. Must be one of: {sorted(VALID_POSTURES)}",
        )
    if "durability" in data and data["durability"] not in VALID_DURABILITIES:
        raise HTTPException(
            400,
            detail=f"Invalid durability '{data['durability']}'. Must be one of: {sorted(VALID_DURABILITIES)}",
        )
    if "sensitivity" in data and data["sensitivity"] not in VALID_SENSITIVITIES:
        raise HTTPException(
            400,
            detail=f"Invalid sensitivity '{data['sensitivity']}'. Must be one of: {sorted(VALID_SENSITIVITIES)}",
        )
...
def _validate_link_enums(entity_type: str, relationship_role: str):
    ...
    if relationship_role not in VALID_RELATIONSHIP_ROLES:
        raise HTTPException(
            400,
            detail=f"Invalid relationship_role '{relationship_role}'. Must be one of: {sorted(VALID_RELATIONSHIP_ROLES)}",
        )
...
async def list_context_notes(
    ...
    if stale is True:
        conditions.append(
            "cn.stale_after IS NOT NULL AND cn.stale_after <= datetime('now')"
        )
    elif stale is False:
        conditions.append(
            "(cn.stale_after IS NULL OR cn.stale_after > datetime('now'))"
        )
    if join_links:
...
async def create_context_note(
    ...
    cached = claim_idempotency_key(
        db, idem_key, body.model_dump(), "/tracker/context-notes"
    )
    if cached == "conflict":
    ...
    """,
        (
            ...
            body.source_id,
            body.source_excerpt,
            body.source_timestamp_start,
            body.source_timestamp_end,
            body.speaker_attribution,
            body.created_by_type,
            body.created_by_person_id,
            body.effective_date,
            body.stale_after,
            body.notes_visibility,
            body.matter_id,
            body.source_communication_id,
            1,
            source_val,
            body.ai_confidence,
    ...
    new_data = data.copy()
    new_data.update(
        {"id": nid, "source": source_val, "created_at": now, "updated_at": now}
    )
    log_event(
        db,
        table_name="context_notes",
        record_id=nid,
        action="create",
        source=write_source,
        new_data=new_data,
...
async def update_context_note(
    note_id: str,
    body: UpdateContextNote,
    request: Request,
    db=Depends(get_db),
    write_source: str = Depends(get_write_source),
):
    old = db.execute(
        "SELECT * FROM context_notes WHERE id = ? AND is_active = 1", (note_id,)
    ).fetchone()
    if not old:
        raise HTTPException(status_code=404, detail="Context note not found")
...
async def delete_context_note(
    ...
    now = datetime.now().isoformat()
    db.execute(
        "UPDATE context_notes SET is_active = 0, updated_at = ? WHERE id = ?",
        (now, note_id),
    )
    log_event(
        db,
...
async def add_context_note_link(
    note_id: str,
    body: CreateContextNoteLink,
    db=Depends(get_db),
    write_source: str = Depends(get_write_source),
):
    note = db.execute(
        "SELECT id FROM context_notes WHERE id = ? AND is_active = 1", (note_id,)
    ).fetchone()
    if not note:
        raise HTTPException(status_code=404, detail="Context note not found")
...
async def remove_context_note_link(
    note_id: str,
    link_id: str,
    db=Depends(get_db),
    write_source: str = Depends(get_write_source),
