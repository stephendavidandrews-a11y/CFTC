...
async def create_directive_document(
    ...
    if not directive:
        raise HTTPException(status_code=404, detail="Policy directive not found")
    doc = db.execute("SELECT id FROM documents WHERE id = ?", (body.document_id,)).fetchone()
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
    ...
    if existing:
        raise HTTPException(status_code=409,
                            detail="This directive is already linked to this document")
    ...
    db.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """, (lid, body.directive_id, body.document_id, body.relationship_type, body.notes, now))

    new_data = body.model_dump()
    new_data.update({"id": lid, "created_at": now})
    log_event(db, table_name="directive_documents", record_id=lid, action="create",
...
async def delete_directive_document(
    ...
    old = db.execute("SELECT * FROM directive_documents WHERE id = ?", (link_id,)).fetchone()
    if not old:
    ...
    log_event(db, table_name="directive_documents", record_id=link_id, action="delete",
