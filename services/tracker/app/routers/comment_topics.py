...
async def create_comment_topic(
    ...
    cached = claim_idempotency_key(
        db, idem_key, body.model_dump(), f"/tracker/matters/{matter_id}/comment-topics"
    )
    if cached == "conflict":
    ...
    """,
        (
            ...
            body.secondary_assignee_person_id,
            body.position_status,
            body.position_summary,
            body.priority,
            body.due_date,
            body.deadline_type,
            body.source_fr_doc_number,
    ...
    new_data.update(
        {"id": tid, "source": source_val, "created_at": now, "updated_at": now}
    )
    log_event(
        db,
...
async def update_comment_topic(
    ...
    old = db.execute(
        "SELECT * FROM comment_topics WHERE id = ?", (topic_id,)
    ).fetchone()
    if not old:
        raise HTTPException(status_code=404, detail="Comment topic not found")
...
async def create_comment_question(
    ...
    topic = db.execute(
        "SELECT id, matter_id FROM comment_topics WHERE id = ?", (topic_id,)
    ).fetchone()
    if not topic:
        raise HTTPException(status_code=404, detail="Comment topic not found")
    ...
    idem_key = request.headers.get("idempotency-key")
    cached = claim_idempotency_key(
        db, idem_key, body.model_dump(), f"/tracker/comment-topics/{topic_id}/questions"
    )
    if cached == "conflict":
        raise HTTPException(409, detail="Idempotency key reused with different payload")
    if cached == "pending":
...
async def update_comment_question(
    ...
    old = db.execute(
        "SELECT * FROM comment_questions WHERE id = ?", (question_id,)
    ).fetchone()
    if not old:
...
async def move_comment_question(
    ...
    target = db.execute(
        "SELECT id, matter_id FROM comment_topics WHERE id = ?", (body.target_topic_id,)
    ).fetchone()
    if not target:
        raise HTTPException(status_code=404, detail="Target topic not found")

    # Verify same matter (can't move questions across matters)
    source_topic = db.execute(
        "SELECT matter_id FROM comment_topics WHERE id = ?", (old["comment_topic_id"],)
    ).fetchone()
    ...
    db.execute(
        "UPDATE comment_questions SET comment_topic_id = ? WHERE id = ?",
    ...
    log_event(
        ...
        old_record=old,
        new_data={
            "comment_topic_id": body.target_topic_id,
            "moved_from": old["comment_topic_id"],
        },
    )
    db.commit()
    return {
