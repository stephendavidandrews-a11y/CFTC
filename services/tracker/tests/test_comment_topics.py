...
def test_create_comment_topic(client, auth_headers, db):
    ...
    resp = client.post(
        f"/tracker/matters/{matter['id']}/comment-topics",
        json=payload,
...
def test_list_comment_topics(client, auth_headers, db):
    ...
    for i in range(3):
        client.post(
            f"/tracker/comment-topics/{topic_id}/questions",
            json={"question_number": str(i + 1), "question_text": f"Question {i + 1}?"},
            headers=auth_headers,
        )

    # List
    resp = client.get(
        f"/tracker/matters/{matter['id']}/comment-topics", headers=auth_headers
    )
    assert resp.status_code == 200
    data = resp.json()
    assert data["total"] == 1
...
def test_update_comment_topic(client, auth_headers, db):
    ...
    resp = client.post(
        f"/tracker/matters/{matter['id']}/comment-topics",
        json={"matter_id": matter["id"], "topic_label": "Original Label"},
        headers=auth_headers,
    )
    topic_id = resp.json()["id"]

    resp = client.put(
        f"/tracker/comment-topics/{topic_id}",
        json={"topic_label": "Updated Label", "position_status": "research"},
        headers=auth_headers,
    )
    assert resp.status_code == 200
...
def test_delete_topic_cascades_questions(client, auth_headers, db):
    ...
    resp = client.post(
        f"/tracker/matters/{matter['id']}/comment-topics",
        json={"matter_id": matter["id"], "topic_label": "To Delete"},
        headers=auth_headers,
    )
    topic_id = resp.json()["id"]
    ...
    # Delete topic
    resp = client.delete(f"/tracker/comment-topics/{topic_id}", headers=auth_headers)
    assert resp.status_code == 200
    assert resp.json()["questions_deleted"] == 2

    # Verify topic is gone
    resp = client.get(f"/tracker/comment-topics/{topic_id}", headers=auth_headers)
...
def test_create_and_update_question(client, auth_headers, db):
    ...
    resp = client.post(
        f"/tracker/matters/{matter['id']}/comment-topics",
        json={"matter_id": matter["id"], "topic_label": "Question Test"},
        headers=auth_headers,
    )
    topic_id = resp.json()["id"]
    ...
    resp = client.put(
        f"/tracker/comment-questions/{qid}",
...
def test_move_question_between_topics(client, auth_headers, db):
    ...
    resp1 = client.post(
        f"/tracker/matters/{matter['id']}/comment-topics",
        json={"matter_id": matter["id"], "topic_label": "Source Topic"},
        headers=auth_headers,
    )
    ...
    resp2 = client.post(
        f"/tracker/matters/{matter['id']}/comment-topics",
        json={"matter_id": matter["id"], "topic_label": "Target Topic"},
        headers=auth_headers,
    )
    ...
    resp = client.post(
        f"/tracker/comment-topics/{source_id}/questions",
        json={"question_number": "1", "question_text": "Movable question"},
        headers=auth_headers,
    )
    qid = resp.json()["id"]
    ...
    # Move it
    resp = client.patch(
        f"/tracker/comment-questions/{qid}/move",
    ...
    row = db.execute(
        "SELECT comment_topic_id FROM comment_questions WHERE id = ?", (qid,)
    ).fetchone()
