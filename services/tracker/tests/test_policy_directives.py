...
def test_create_policy_directive(client, auth_headers, db):
    ...
    resp = client.post("/tracker/policy-directives", json=payload,
...
def test_list_policy_directives_with_filters(client, auth_headers, db):
    ...
    for i, status in enumerate(["not_started", "in_progress", "implemented"]):
        client.post("/tracker/policy-directives",
                    json={"source_document": "Test Doc",
    ...
    # Filter by status
    resp = client.get("/tracker/policy-directives",
...
def test_update_policy_directive(client, auth_headers, db):
    ...
    resp = client.post("/tracker/policy-directives",
                       json={"source_document": "Test",
                             "source_document_type": "executive_order",
                             "directive_label": "Update Test"},
                       headers=auth_headers)
    did = resp.json()["id"]

    resp = client.put(f"/tracker/policy-directives/{did}",
                      json={"implementation_status": "scoping",
...
def test_get_directive_with_linked_matters(client, auth_headers, db):
    ...
    # Link matter
    client.post("/tracker/directive-matters",
                json={"directive_id": did, "matter_id": matter["id"],
                      "relationship_type": "implements"},
                headers=auth_headers)

    # Get directive
    resp = client.get(f"/tracker/policy-directives/{did}",
                      headers=auth_headers)
    assert resp.status_code == 200
    data = resp.json()
...
def test_delete_directive_cascades_links(client, auth_headers, db):
    ...
    matter = seed_matter(db)
    resp = client.post("/tracker/policy-directives",
                       json={"source_document": "Delete Test",
    ...
    resp = client.delete(f"/tracker/policy-directives/{did}",
                         headers=auth_headers)
    assert resp.status_code == 200
...
def test_link_directive_to_matter(client, auth_headers, db):
    ...
    resp = client.post("/tracker/directive-matters",
                       json={"directive_id": did, "matter_id": matter["id"],
                             "relationship_type": "implements"},
                       headers=auth_headers)
    assert resp.status_code == 200
...
def test_duplicate_link_rejected(client, auth_headers, db):
    ...
    resp1 = client.post("/tracker/directive-matters", json=payload,
                        headers=auth_headers)
    assert resp1.status_code == 200

    resp2 = client.post("/tracker/directive-matters", json=payload,
                        headers=auth_headers)
...
def test_unlink_directive_from_matter(client, auth_headers, db):
    ...
    link = client.post("/tracker/directive-matters",
                       json={"directive_id": did, "matter_id": matter["id"]},
                       headers=auth_headers)
    lid = link.json()["id"]

    resp = client.delete(f"/tracker/directive-matters/{lid}",
                         headers=auth_headers)
    assert resp.status_code == 200
...
def test_reverse_lookup_matter_directives(client, auth_headers, db):
    ...
    resp = client.get(f"/tracker/matters/{matter['id']}/directives",
                      headers=auth_headers)
    assert resp.status_code == 200
    assert resp.json()["total"] == 1
