...
def move_item(db, communication_id: str, item_id: str,
              from_bundle_id: str, to_bundle_id: str,
              item: dict) -> dict:
    ...
    write_audit(db, communication_id, from_bundle_id, item_id, "move_item", {
    ...
    return {"status": "ok", "original_item_id": item_id,
            "new_item_id": new_item_id, "moved_to_bundle": to_bundle_id}


def create_bundle(db, communication_id: str,
                  ...
                  intelligence_notes: str | None = None) -> dict:
    ...
    """, (
        bundle_id, communication_id, bundle_type,
        target_matter_id, target_matter_title,
        rationale, intelligence_notes, next_sort,
    ))

    write_audit(db, communication_id, bundle_id, None, "create_bundle", {
...
def merge_bundles(db, communication_id: str,
                  source_bundle_id: str, target_bundle_id: str,
    ...
    for si in source_items:
        ...
        db.execute("""
            ...
            SELECT ?, ?, item_type, ?, proposed_data, original_proposed_data,
                   ...
                   ?, ?, datetime('now'), datetime('now')
            FROM review_bundle_items WHERE id = ?
        """, (new_id, target_bundle_id,
              si["status"] if si["status"] in ("accepted", "edited") else "proposed",
              next_sort, source_bundle_id, si["id"]))
        next_sort += 1
        moved_count += 1
    ...
    write_audit(db, communication_id, target_bundle_id, None, "merge_bundles", {
    ...
    return {"status": "ok", "target_bundle_id": target_bundle_id,
            "source_bundle_id": source_bundle_id,
            "items_moved": moved_count}
