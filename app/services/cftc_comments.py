...
class CftcCommentsClient:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        if self._client is None or self._client.is_closed:
            self._client = httpx.AsyncClient(
                ...
                headers={
                    "User-Agent": "CFTC-Comment-Analysis-Tool/1.0 (personal research tool)",
    ...
    def _parse_releases_page(self, html: str) -> list[CftcRuleEntry]:
        ...
        Since the ASP.NET HTML is complex, we use regex-based extraction
        as a more reliable approach than pure HTML parsing for this site.
        """
        rules = []

        # Find all "View Comments" links — each represents one rulemaking
        view_pattern = re.compile(
            r'CommentList\.aspx\?id=(\d+)',
            re.IGNORECASE,
        )

        # Find all rulemaking blocks — look for FR citations and titles
        # Pattern: FR citation followed by title text, then dates
        block_pattern = re.compile(
            r'(\d+\s+FR\s+\d+).*?'   # FR citation
            r'(?:CommentList\.aspx\?id=(\d+))',  # release ID
            re.DOTALL | re.IGNORECASE,
        )

        # Simpler approach: find all release IDs and associated text
        # Split the HTML by "View Comments" links
        segments = re.split(r'(?=CommentList\.aspx\?id=)', html)

        for segment in segments:
            id_match = re.search(r'CommentList\.aspx\?id=(\d+)', segment)
            if not id_match:
                continue

            release_id = int(id_match.group(1))

            # Look backwards in the HTML for the title (within a reasonable range)
            # Find the nearest preceding large text block
            # Extract FR citation
            fr_match = re.search(r'(\d+\s+FR\s+\d+)', segment)
            fr_citation = fr_match.group(1) if fr_match else ""

            # Extract dates
            date_pattern = r'(\d{1,2}/\d{1,2}/\d{4})'
            dates = re.findall(date_pattern, segment)
            ...
            if dates:
                try:
                    if len(dates) >= 2:
                        open_date = datetime.strptime(dates[0], "%m/%d/%Y").date()
                        closing_date = datetime.strptime(dates[1], "%m/%d/%Y").date()
                    elif len(dates) == 1:
                        closing_date = datetime.strptime(dates[0], "%m/%d/%Y").date()
                except ValueError:
                    pass

            # Extract title — usually the longest text segment
            # Remove HTML tags and get clean text
            clean = re.sub(r'<[^>]+>', ' ', segment)
            clean = re.sub(r'\s+', ' ', clean).strip()

            # Title is typically the first substantial sentence
            title_candidates = [s.strip() for s in clean.split('.') if len(s.strip()) > 20]
            title = title_candidates[0] if title_candidates else f"CFTC Release #{release_id}"
    ...
    def _parse_comment_list(self, html: str, release_id: int) -> list[CftcComment]:
        ...
        for row_html in rows:
            ...
            # Parse cells for commenter info and dates
            for cell_text in clean_cells:
                ...
                # Skip cells that are just the comment number
                if re.match(r'^\d+$', cell_text.strip()):
                    continue

                # Assign to name or org
