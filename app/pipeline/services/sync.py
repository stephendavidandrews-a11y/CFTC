...
UA_STAGE_MAP = {
    ...
    "Final Rule Stage":    ("final_rule", "drafting"),
...
def _determine_stage(
    ...
    Priority logic:
      1. FR docs showing withdrawal → None (don't change stage)
      2. FR docs showing final/ifr/dfr → "published"
      3. UA stage (authoritative for lifecycle position) — e.g., if UA says
         "Final Rule Stage" but FR only has NPRMs, trust UA.
      4. FR docs showing NPRM-phase activity → comment_period/comment_analysis
      5. Item type fallback
    """
    sorted_docs = sorted(
        fr_docs, key=lambda d: d.get("publication_date", ""), reverse=True
    )

    # First pass: check FR docs for conclusive terminal actions
    # (withdrawal, final rule published). These override everything.
    fr_nprm_stage = None
    for doc in sorted_docs:
        action = _classify_fr_action(doc.get("action", ""))
        ...
        # Corrections/delays are not stage-changing — skip
        if action in ("correction", "delay"):
            continue

        # NPRM-phase activity — note it but don't return yet
        if action in ("nprm", "anprm", "extension", "reopening",
                      "supplemental", "rfc") and fr_nprm_stage is None:
            close_date = doc.get("comments_close_on")
            if close_date:
                try:
                    close = datetime.strptime(close_date, "%Y-%m-%d").date()
                    fr_nprm_stage = "comment_period" if close > date.today() else "comment_analysis"
                except ValueError:
                    fr_nprm_stage = "comment_period"
            else:
                fr_nprm_stage = "comment_period"

    # No terminal FR action found. Check UA stage — it's the authoritative
    # source for where the rulemaking IS in its lifecycle.
    if ua_entry and ua_entry.get("stage"):
        stage = ua_entry["stage"]
        if "Completed" in stage:
            return "published"

        # If UA says "Final Rule Stage", the rulemaking has advanced beyond
        # the NPRM phase. Trust UA over old FR NPRM docs.
        for key, (_, default_stage) in UA_STAGE_MAP.items():
            if key.lower() in stage.lower():
                # For "Final Rule Stage" → returns "drafting" (first stage
                # in the final_rule pipeline). For "Proposed Rule Stage" →
                # returns "drafting" but FR NPRM data below may be more specific.
                if "final rule" in stage.lower():
                    return default_stage
                # For proposed rule stage, prefer FR-based stage if available
                if fr_nprm_stage:
                    return fr_nprm_stage
                return default_stage

    # No UA data — use FR NPRM stage if we found one
    if fr_nprm_stage:
        return fr_nprm_stage

    # Check item_type for fallback
    if item_type == "final_rule":
        return "drafting" if not sorted_docs else "published"
