...
def get_kanban(conn, module: str, item_type=None, status="active") -> dict:
    ...
    stages = conn.execute(
        f"""SELECT DISTINCT item_type, stage_key, stage_label, stage_color, stage_order
            FROM stage_templates
    ...
    # Deduplicate by (stage_key, stage_label) so that stages with the same
    # key but different labels (e.g. "Drafting" vs "Final Rule Drafting")
    # remain separate columns. Track which item_types belong to each column.
    seen = {}  # (stage_key, stage_label) -> column dict
    unique_stages = []
    for s in stages:
        dedup_key = (s["stage_key"], s["stage_label"])
        if dedup_key not in seen:
            col = dict(s)
            col["_item_types"] = {s["item_type"]}
            seen[dedup_key] = col
            unique_stages.append(col)
        else:
            seen[dedup_key]["_item_types"].add(s["item_type"])
    ...
    # Group by stage — match items to columns by stage_key + item_type
    columns = []
    total = 0
    for stage in unique_stages:
        stage_items = [
            i for i in enriched
            if i["current_stage"] == stage["stage_key"]
            and i["item_type"] in stage["_item_types"]
        ]
        columns.append({
...
def list_items_simple(conn, status="active") -> list[dict]:
    """Return a lightweight list of pipeline items for dropdowns.

    Returns only id, title, rin, docket_number, item_type.
    Excludes items that already have a linked work project.
    """
    from app.work.db import get_connection as get_work_conn, attach_pipeline

    # Get IDs already linked to a work project
    linked_ids = set()
    try:
        wconn = get_work_conn()
        rows = wconn.execute(
            "SELECT linked_pipeline_id FROM projects WHERE linked_pipeline_id IS NOT NULL"
        ).fetchall()
        linked_ids = {r["linked_pipeline_id"] for r in rows}
        wconn.close()
    except Exception:
        pass  # If work DB unavailable, show all items

    rows = conn.execute(
        """SELECT id, title, rin, docket_number, item_type
           FROM pipeline_items
           WHERE status = ?
           ORDER BY title""",
        (status,),
    ...
    return [dict(r) for r in rows if r["id"] not in linked_ids]


def create_project_from_item(conn, item_id: int, data: dict) -> dict:
    """Create a work project pre-populated from a pipeline item.

    Args:
        conn: pipeline DB connection
        item_id: pipeline item ID
        data: optional overrides (priority_label, apply_template)

    Returns:
        The created work project dict.

    Raises:
        ValueError: if item not found
        LookupError: if a project already exists for this item
    """
    from app.work.db import get_connection as get_work_conn, attach_pipeline
    from app.work.services.projects import create_project
    ...
    # Open work DB and check for existing linked project
    wconn = get_work_conn()
    attach_pipeline(wconn)
    try:
        existing = wconn.execute(
            "SELECT id FROM projects WHERE linked_pipeline_id = ?", (item_id,)
        ).fetchone()
        if existing:
            raise LookupError(f"Project already exists for pipeline item {item_id}")

        # Build project data from pipeline item
        project_data = {
            "title": item["title"],
            "description": item.get("description"),
            "project_type": "rulemaking",
            "linked_pipeline_id": item_id,
            "linked_docket": item.get("docket_number"),
            "lead_attorney_id": item.get("lead_attorney_id"),
            "priority_label": data.get("priority_label", item.get("priority_label", "medium")),
            "apply_template": data.get("apply_template", True),
        }

        result = create_project(wconn, project_data)
        return result
    finally:
        wconn.close()
