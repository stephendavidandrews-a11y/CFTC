...
PIPELINE_TABLES = [
    ...
    # -----------------------------------------------------------------------
    # 20. interagency_contacts
    # -----------------------------------------------------------------------
    ("interagency_contacts", """
    CREATE TABLE IF NOT EXISTS interagency_contacts (
        id                  INTEGER PRIMARY KEY AUTOINCREMENT,
        name                TEXT NOT NULL,
        title               TEXT,
        ...
        relationship_status TEXT DEFAULT 'acquaintance',
    ...
    # -----------------------------------------------------------------------
    # 21. interagency_rulemakings
    # -----------------------------------------------------------------------
    ("interagency_rulemakings", """
    CREATE TABLE IF NOT EXISTS interagency_rulemakings (
        id                  INTEGER PRIMARY KEY AUTOINCREMENT,
        title               TEXT NOT NULL,
        agency              TEXT NOT NULL,
        rulemaking_type     TEXT,
        topics              TEXT DEFAULT '[]',
        status              TEXT DEFAULT 'active',
        ...
        summary             TEXT,
        cftc_position       TEXT,
        impact_on_cftc_work TEXT,
        linked_pipeline_ids TEXT DEFAULT '[]',
        created_at          TEXT DEFAULT (datetime('now')),
        updated_at          TEXT DEFAULT (datetime('now'))
    )
    """),
...
PIPELINE_INDEXES = [
    ...
    # interagency_contacts
    "CREATE INDEX IF NOT EXISTS idx_ic_agency ON interagency_contacts(agency)",
    "CREATE INDEX IF NOT EXISTS idx_ic_status ON interagency_contacts(relationship_status)",

    # interagency_rulemakings
    "CREATE INDEX IF NOT EXISTS idx_ir_agency ON interagency_rulemakings(agency)",
    "CREATE INDEX IF NOT EXISTS idx_ir_status ON interagency_rulemakings(status)",

    # email_log
    "CREATE INDEX IF NOT EXISTS idx_el_type ON email_log(email_type)",
    "CREATE INDEX IF NOT EXISTS idx_el_sent ON email_log(sent_at)",

    # ai_processing_log
    "CREATE INDEX IF NOT EXISTS idx_apl_type ON ai_processing_log(processing_type)",
]
...
def _run_pipeline_migrations(conn: sqlite3.Connection):
    """Run idempotent ALTER TABLE migrations for existing tables."""
    # Team member profile extensions
    tm_cols = {r[1] for r in conn.execute("PRAGMA table_info(team_members)").fetchall()}
    new_tm_cols = {
        "background_summary": "TEXT",
        "working_style": "TEXT DEFAULT 'balanced'",
        "communication_preference": "TEXT DEFAULT 'email'",
        "current_capacity": "TEXT DEFAULT 'available'",
        "strengths": "TEXT DEFAULT '[]'",
        "growth_areas": "TEXT DEFAULT '[]'",
        "personal_context": "TEXT",
        "recent_wins": "TEXT DEFAULT '[]'",
    }
    for col_name, col_def in new_tm_cols.items():
        if col_name not in tm_cols:
            conn.execute(f"ALTER TABLE team_members ADD COLUMN {col_name} {col_def}")
            logger.info(f"Pipeline migration: added {col_name} to team_members")
    conn.commit()
