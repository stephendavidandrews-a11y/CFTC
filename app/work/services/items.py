...
from app.work.services.tasks import list_tasks_for_work_item
...
def _enrich_item(row: sqlite3.Row, conn: sqlite3.Connection) -> dict:
    ...
    d["children"] = []
    d["tasks"] = list_tasks_for_work_item(conn, d["id"])
    d["depth"] = d.get("depth", 0)

    # Computed attention indicators
    today = date.today()
    d["days_since_update"] = None
    d["days_until_due"] = None
    d["needs_attention"] = False

    if d.get("last_update_date"):
        try:
            last_update = date.fromisoformat(d["last_update_date"])
            d["days_since_update"] = (today - last_update).days
    ...
    if status != "completed":
        d["needs_attention"] = (
            status in ("waiting_on_stephen", "in_review")
            or (d["days_since_update"] is not None and d["days_since_update"] > 7)
            or (d["days_until_due"] is not None and d["days_until_due"] <= 3)
        )

    # Ensure new fields have defaults
    d.setdefault("context", None)
    d.setdefault("last_update_notes", None)
    d.setdefault("last_update_date", None)
