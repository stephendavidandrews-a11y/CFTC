...
WORK_TABLES = [
    ...
    CREATE INDEX IF NOT EXISTS idx_task_work_item ON tasks(work_item_id);
    """),
...
def init_work_schema(conn: sqlite3.Connection) -> list[str]:
    ...
    # Add parent_id to tasks for sub-task nesting
    cols = [r[1] for r in conn.execute("PRAGMA table_info(tasks)").fetchall()]
    if "parent_id" not in cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN parent_id INTEGER REFERENCES tasks(id) ON DELETE CASCADE")
        conn.commit()
        logger.info("Work schema migration: added parent_id to tasks")

    # Create parent_id index (safe now that column definitely exists)
    ...
    for col_name, col_def in new_proj_cols.items():
        if col_name not in proj_cols:
            ...
            logger.info(f"Work migration: added {col_name} to projects")
    ...
    for col_name, col_def in new_wi_cols.items():
        if col_name not in wi_cols:
            conn.execute(f"ALTER TABLE work_items ADD COLUMN {col_name} {col_def}")
            logger.info(f"Work migration: added {col_name} to work_items")

    # ── Manager notes extensions ───────────────────────────────────
    mn_cols = {r[1] for r in conn.execute("PRAGMA table_info(manager_notes)").fetchall()}
    new_mn_cols = {
        "context_type": "TEXT DEFAULT 'general'",
        "processed": "INTEGER DEFAULT 0",
        "ai_insights": "TEXT",
    }
    for col_name, col_def in new_mn_cols.items():
        if col_name not in mn_cols:
            conn.execute(f"ALTER TABLE manager_notes ADD COLUMN {col_name} {col_def}")
            logger.info(f"Work migration: added {col_name} to manager_notes")
