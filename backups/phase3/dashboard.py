...
async def get_dashboard(db=Depends(get_db)):
    ...
    for row in db.execute(
        "SELECT status, COUNT(*) as count FROM matters WHERE status != 'closed' GROUP BY status"
    ):
    ...
    matters_by_priority = {}
    for row in db.execute(
        "SELECT priority, COUNT(*) as count FROM matters WHERE status != 'closed' GROUP BY priority"
    ):
        matters_by_priority[row["priority"]] = row["count"]

    # Total open matters
    total_open = db.execute(
        "SELECT COUNT(*) as c FROM matters WHERE status != 'closed'"
    ).fetchone()["c"]
